...
{
	var fun = function(){
		//location.href = APP+'/'+CURR_GROUP+'/'+CURR_MODULE+'/add';
                location.href = '/'+CURR_GROUP+'/'+CURR_MODULE+'/modify/';
	};
	
	setTimeout(fun,1);
}

function editData(id,pk)
{
	if(isNaN(id))
	...
	var fun = function(){
		location.href = APP+'/'+CURR_GROUP+'/'+CURR_MODULE+'/edit/'+pk+'/' + id;
	};
	
	setTimeout(fun,1);
}

function modifyData(id,pk)
{
	if(isNaN(id))
		id = $("#" + id + " input:checked[name='key']").eq(0).val();
	...
	var fun = function(){
		location.href = '/'+CURR_GROUP+'/'+CURR_MODULE+'/modify/'+pk+'/' + id;
	};
...
function export(){
	...
	$.ajax({
		url: APP + '?' + VAR_MODULE + '=' + CURR_MODULE + '&' + VAR_ACTION + '=export',
		type:"POST",
		cache: false,
		data:query,
		dataType:"json",
