	...
	<div class="fb-body">
		<form method='post' id="form" name="form"  action="<?php echo U(MODULE_NAME.'/update');?>" >
			<table cellpadding="4" cellspacing="0" cellspacing="1"  border="0" class="table-form">
				...
				<tr>
					<th  width="200">名称</th> <td><textarea name="title" cols="80" rows="2" ><?php echo ($res["title"]); ?></textarea></td>
				</tr>
					...
					<td>
						<div align="left">
							...
							<select name="category"  style="width:150px">
								<?php if(is_array($category)): foreach($category as $key=>$vo): ?><optgroup label="<?php echo ($vo["name_en"]); ?> <?php echo ($vo["name"]); ?>">
									<?php if(is_array($vo["sub"])): foreach($vo["sub"] as $key=>$cat_sub): if(cat_sub.id == $res.category): ?><option value ="<?php echo ($cat_sub["id"]); ?>" selected ><?php echo ($cat_sub["name"]); ?></option>
									<?php else: ?>
									<option value ="<?php echo ($cat_sub["id"]); ?>"><?php echo ($cat_sub["name"]); ?></option><?php endif; endforeach; endif; ?>
				...
				<tr>
					<th>简介</th>
					<td><textarea  name="intro" cols="80" rows="8" ><?php echo ($res["intro"]); ?></textarea></td>
				</tr>

				<tr>
					<th>正文</th>
					<td><textarea  name="content"  ><?php echo ($res["content"]); ?></textarea></td>
				</tr>

				<tr>
					<th  width="200">发布时间</th>
					<td><input class="Wdate" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" name="time"  value="<?php echo ($res["time"]); ?>" /></td>
				</tr>
					...
					<td>
						<div id="uploaded">
							<div class="imgorder" id="img0">
								...
								<input type="hidden" id="imgval0" name="img" value="<?php echo ($res["img"]); ?>">
				...
				<tr>
					<th>状态</th>
					<td>
						上线<input type="radio"  name="status" value="1" <?php if(($res["status"] == 1)): ?>checked<?php endif; ?> />
						下线<input type="radio"  name="status" value="0" <?php if(($res["status"] == 0)): ?>checked<?php endif; ?> />
					</td>
				</tr>
				...
				<tr class="act">
					...
					<td>
						...
						<a href="<?php echo U(MODULE_NAME.'/index');?>" class="submit_btn">取消</a>
					</td>
				</tr>
