...
<body>
<div class="cw-body">
    <div class="fb-title"><div><p><span>图文 > <php>if(isset($vo['id'])) echo '编辑'; else echo '添加'; </php></if></span></p></div></div>
    <div class="fb-body">
        <form method='post' id="form1" name="form1" action="{:U(MODULE_NAME.'/modify')}">
            <table cellpadding="4" cellspacing="0" cellspacing="1"  border="0" class="table-form">
                ...
                <tr>
                    <th  width="200">图文类型</th>
                    <td>
                            <?php foreach($atype as $type): ?>
                            <input type="checkbox" name="type[]" value="{$type.id}"  <?php if( $type['id'] & intval($vo['type']) ){ echo 'checked'; } ?> />{$type.name}
                            <?php endforeach; ?>
                    </td>
                </tr>
