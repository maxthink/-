    ...
    <div class="fb-body">
        ...
        <form method='post' id="form" name="form" action="<if condition="($Think.const.ACTION_NAME eq 'add')"> {:U(MODULE_NAME.'/save')}<else />{:U(MODULE_NAME.'/update')}</if>">
            <table cellpadding="4" cellspacing="0" cellspacing="1"  border="0" class="table-form">
                ...
                <tr>
                    <th>是否显示</th>
                    <td>显示<input type="radio"  name="is_show" value="1" <if condition="($vo.is_show eq 1)"> checked</if>/>不显示<input type="radio"  name="is_show" value="0" <if condition="($vo.is_show eq 0)"> checked</if>/></td>
                </tr>
                <tr class="act">
                    <th>&nbsp;</th>
                    <td>
