...
<div class="nav-sub-container">
    <div class="nav-sub-item current"><a href="/m">综合</a></div>
    
    <foreach name="types" item="type">
        <div class="nav-sub-item "><a href="{:U('')}/type/{$type.id}">{$type.name}</a></div>
    </foreach>
    
    <div id="more" class="nav-sub-item " style="position:absolute; "><a href="/m">更多</a></div>
</div>
        ...
        <div class="article-item">
            <a href="{$val.url}">
                <div class="item-cover">
                    <img src="/Public/wx/default/image/logo.png" data-original="/{$val.cover_dir|getimg=150}" title="{$val.title}" style="display: inline; height:76px;" class="lazy" >
                </div>
                <div class="item-text">
                ...
                if(res.status==1){
                    ...
                    $.each(res.list, function(key,val){
                        var html = '<div class="article-item">'+
                            '<a href="'+val.url+'">'+
                                '<div class="item-cover">'+
                                    '<img src="/'+val.cover_dir+'" title="'+val.title+'" style="display: inline; height: 76px;" class="lazy">'+
                                '</div>'+
                                '<div class="item-text">'+
                                    '<div class="item-title">'+val.title+'</div>'+
                                    '<div class="item-content">'+
                                        '<div class="item-summary"></div>'+
                                        '<div class="item-timeline">'+formatDate(val.update_time*1000)+'</div>'+
                                    '</div>'+
                                '</div>'+
                            '</a>'+
                        '</div>';
