	...
	<div class="fb-title"><div><p><span>菜单 > 编辑</span></p></div></div>
		<div class="fb-body">
			...
			<table cellpadding="4" cellspacing="0" cellspacing="1"  border="0" class="table-form">
					...
					<td>
						<select id="role_id" name="group" onchange="" ondblclick="" class="">
							<foreach name='glist' item='v' key='k'>
								<option value="{$v.id}" <if condition="($v[id] eq $vo[group])"> selected="selected"</if>  >{$v.name}</option>
							</foreach>
						</select>
					</td>
				</tr>
				...
				<tr>
					<th  width="200">身份证</th>
					<td><input type="text"  name="identityid"  value="{$vo.identityid}" /></td>
				</tr>
				<tr>
					<th>状态</th>
					<td>
